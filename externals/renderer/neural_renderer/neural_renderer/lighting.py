...
def lighting(faces, textures, intensity_ambient=0.5, intensity_directional=0.5,
    ...
    if color_ambient.shape[1]<texture_channel:
        color_ambient=torch.cat((color_ambient, 
            torch.ones(color_ambient.shape[0], texture_channel-color_ambient.shape[1]).cuda()), dim=1)
    if color_directional.shape[1]<texture_channel:
        color_directional=torch.cat((color_directional, 
            torch.ones(color_directional.shape[0], texture_channel-color_directional.shape[1]).cuda()), dim=1)

    # create light
    light = torch.zeros(bs, nf, texture_channel, dtype=torch.float32).to(device)
