...
__global__ void forward_texture_sampling_cuda_kernel(
    ...
    if (face_index >= 0) {
        ...
        */
        ...
        const scalar_t* texture = &textures[(bn * nf + face_index) * ts * ts * ts * texture_channel];
        scalar_t* pixel = &rgb_map[i * texture_channel];
	...
	// initialize pixel with a large size, we do not support texture_channel>256
        ...
        for (int pn = 0; pn < 8; pn++) {
            ...
            for (int k = 0; k < texture_channel; k++)
                new_pixel[k] += w * texture[isc * texture_channel + k];
...
__global__ void backward_textures_cuda_kernel(
    ...
    if (0 <= face_index) {
        ...
        scalar_t* grad_texture = &grad_textures[(bn * nf + face_index) * ts * ts * ts * texture_channel];
        ...
        for (int pn = 0; pn < 8; pn++) {
            ...
            scalar_t* grad_texture_p = &grad_texture[isc * texture_channel];
            scalar_t* grad_rgb_map_p = &grad_rgb_map[i * texture_channel];
            for (int k = 0; k < texture_channel; k++)
...
std::vector<at::Tensor> forward_texture_sampling_cuda( at::Tensor faces,
    ...
    const auto texture_channel = textures.size(5);
    const int threads = 512;
...
at::Tensor backward_textures_cuda(
    ...
    const auto texture_channel = grad_textures.size(5);
    const int threads = 512;
    const dim3 blocks ((batch_size * image_size * image_size - 1) / threads + 1);
