...
import externals.renderer.neural_renderer.neural_renderer.cuda.rasterize as rasterize_cuda
...
class RasterizeFunction(Function):
    ...
    def forward(ctx, faces, textures, image_size, near, far, eps, background_color,
                return_rgb=False, return_alpha=False, return_depth=False, texture_channel=3):
        '''
        Forward pass
        '''
        ...
        if ctx.return_rgb:
            rgb_map = torch.cuda.FloatTensor(ctx.batch_size, ctx.image_size, ctx.image_size, texture_channel).fill_(0)
...
def rasterize_rgbad(
    ...
    if anti_aliasing:
        ...
        rgb, alpha, depth = Rasterize(
            image_size * 2, near, far, eps, background_color, return_rgb, return_alpha, return_depth, texture_channel)(*inputs)
    else:
        rgb, alpha, depth = Rasterize(
            image_size, near, far, eps, background_color, return_rgb, return_alpha, return_depth, texture_channel)(*inputs)
