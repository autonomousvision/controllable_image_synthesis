...
class Discriminator(nn.Module):
    def __init__(self, input_nc, nlabels):
        super(Discriminator, self).__init__()

        self.model = nn.Sequential(
                FirstResBlockDiscriminator(input_nc, DISC_SIZE, stride=2),
        ...
        self.fc = nn.Linear(DISC_SIZE, nlabels)
    ...
    def forward(self, x, z=None):
        x = x[:,:self.input_nc]
        out = self.fc(self.model(x).view(-1,DISC_SIZE))

        if self.nlabels>1:
            bs = out.shape[0]
            index = torch.arange(0,bs).long().to(x.device)
            out = out[index, z]
