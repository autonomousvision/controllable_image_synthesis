...
def get_statistics(data_loader, model, device=None, batch_size=32, resize=False, n_samples=None):
...
class FIDEvaluator(object):
  def __init__(self, device=None, batch_size=32, resize=False, n_samples=None):
    self.device = device
  ...
  def get_statistics(self, data_loader):
    return get_statistics(data_loader, self.model, device=self.device, batch_size=self.batch_size,
  ...
  def initialize_target(self, target_loader, cache_file=None):
    if cache_file is not None:
      if os.path.isfile(cache_file):
        ...
        self.mu_target, self.sigma_target = self.get_statistics(target_loader)
  ...
  def is_initialized(self):
    return not any([self.mu_target is None, self.sigma_target is None])
  
  def get_fid(self, data_loader):
    assert self.is_initialized()
    mu, sigma = self.get_statistics(data_loader)
